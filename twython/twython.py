...
class Twython(object):
    def __init__(self, twitter_token = None, twitter_secret = None, oauth_token = None, oauth_token_secret = None, headers=None, callback_url=None, client_args={}, proxy=""):
    ...
    def constructApiURL(base_url, params):
        return base_url + "?" + "&".join(["%s=%s" %(Twython.unicode2utf8(key), urllib.quote_plus(Twython.unicode2utf8(str(value)))) for (key, value) in params.iteritems()])

    @staticmethod
    def shortenURL(url_to_shorten, shortener = "https://is.gd/api.php", query = "longurl"):
    ...
    def isListMember(self, list_id, id, username, version = 1):
        ...
        """
        try:
            resp, content = self.client.request("https://api.twitter.com/%d/%s/%s/members/%s.json" % (version, username, list_id, `id`), headers = self.headers)
            return simplejson.loads(content.decode('utf-8'))
        except HTTPError, e:
    ...
    def isListSubscriber(self, username, list_id, id, version = 1):
        ...
        """
        try:
            resp, content = self.client.request("https://api.twitter.com/%d/%s/%s/following/%s.json" % (version, username, list_id, `id`), headers = self.headers)
            return simplejson.loads(content.decode('utf-8'))
        except HTTPError, e:
            raise TwythonError("isListMember() failed with a %d error code." % e.code, e.code)
    ...
    def updateProfileBackgroundImage(self, filename, tile="true", version = 1):
        ...
        try:
            ...
            r = urllib2.Request("https://api.twitter.com/%d/account/update_profile_background_image.json?tile=%s" % (version, tile), body, headers)
    ...
    def updateProfileImage(self, filename, version = 1):
        ...
        try:
            ...
            content_type, body = Twython.encode_multipart_formdata(fields, files)
            headers = {'Content-Type': content_type, 'Content-Length': str(len(body))}
            r = urllib2.Request("https://api.twitter.com/%d/account/update_profile_image.json" % version, body, headers)
            return urllib2.urlopen(r).read()
        except HTTPError, e:
